...
/* To prepare
 * 1) Install MiniCore in additional boards; (copy into
 * File->Preferences->Additional Boards Manager URLs)
 *     https://mcudude.github.io/MiniCore/package_MCUdude_MiniCore_index.json
 * 2) Then add MiniCore by searching and installing (Tools->Board->Board
 * Manager) 3) Install Adafruit_GFX and Adafruit_SSD1306 libraries
 ...
 *  2) Set programmer of choice, e.g.'Arduino as ISP (MiniCore)', 'USB ASP',
 * etc, and set correct port. 3) Burn bootloader (to set fuses correctly) 4)
 * Compile and upload
 */
...
bool heat(byte maxTemp) {
  ...
  /*  Heater follows industry reflow graph. Slow build-up to 'warmUp' temp.
   * Rapid ascent to 'maxTemp'. Then descent to room temperature.
   */
  // byte maxTemp; //Declared in function call
  byte maxPWM =
      0.70 *
      maxTemp; // Temperatures (in PWM / 255) influenced by paste temperature
  ...
  unsigned long eTime =
      (millis() / 1000) + (8 * 60); // Used to store the end time of the heating
                                    // process, limited to 8 mins
  ...
  while (1) {
    ...
    if (t < warmUpTemp) { // Warm Up Section
      ...
      if (v < vMin && pwmVal > 1) {
        pwmVal = pwmVal - 2;
      } // Reduce PWM Value if V drops too low but not unless it is still above
        // 1 (avoid overflow/underflow)
    ...
    display.drawBitmap(112, 3, heat_animate, heat_animate_width,
                       heat_animate_height, SSD1306_WHITE);
    display.fillRect(0, 3, heat_animate_width,
